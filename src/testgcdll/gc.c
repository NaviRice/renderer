...
int initgame(void){
	...
	entity_t crank = {0};
		crank.name = "crank"; //todo figure out if i need to strdup here or not
		crank.anglevel[0] = 10.0;
		crank.needsmatupdate =1;
		crank.scale = 1.0;
		crank.pos[2] = -5.0;
		int crankid = ec->entity_addRINT(crank);
	...
	entity_t e ={0};
		...
		e.nextthink = ec->now;
		entity_t *bunny = ec->entity_addRPOINT(e);
